...
<div class="page-wrapper">
    <div class="content">
        ...
        <div class="row">
            <div class="col-lg-8">
                <div class="card">
                    <div class="card-header"><h5 class="mb-0 fw-bold">Compose Message</h5></div>
                    <div class="card-body">
                        <form action="{{ route('admin.sms.send') }}" method="POST" id="smsForm">
                            ...
                            <div class="mb-4">
                                <div class="form-check form-switch">
                                    <input type="checkbox" name="use_parent_phone" id="useParentPhone" value="1" class="form-check-input" checked>
                                    <label class="form-check-label" for="useParentPhone">Send to parent/guardian phone (recommended)</label>
                                </div>
                            </div>
                            ...
                            <div class="mb-4">
                                ...
                                <textarea name="message" id="messageBody" rows="4" class="form-control" maxlength="320" required oninput="updateCharCount()">{{ old('message') }}</textarea>
                                <div class="d-flex justify-content-between mt-1">
                                    <span class="text-muted small">Variables: {{student_name}}, {{balance}}, {{class}}, {{term}}, {{school}}</span>
                                    <span class="text-muted small" id="charCount">0 / 320</span>
                                </div>
                            </div>
                            ...
                            <div class="d-flex gap-2">
                                <button type="submit" class="btn btn-primary">
                                    <i class="fe fe-send me-1"></i> Send SMS
                                </button>
            ...
            <div class="col-lg-4">
                <div class="card">
                    <div class="card-header"><h6 class="mb-0 fw-bold">SMS Balance</h6></div>
                    <div class="card-body">
                        <div class="text-center">
                            <h3 class="mb-1">{{ $balance['balance'] ?? 0 }}</h3>
                            <p class="text-muted mb-0">Units Remaining</p>
                            @if(($balance['balance'] ?? 0) < 100)
                                <div class="alert alert-warning mt-3 mb-0 py-2">
                                    <i class="fe fe-alert-triangle me-1"></i> Low balance! Please top up.
                                </div>
                            @endif
                        </div>
                    </div>
                ...
                <div class="card mt-3">
                    ...
                    <div class="card-body">
                        <ul class="list-unstyled mb-0 small text-muted">
                            <li class="mb-2"><i class="fe fe-check-circle me-1 text-success"></i> Messages are limited to 320 characters.</li>
                            <li class="mb-2"><i class="fe fe-check-circle me-1 text-success"></i> Use variables to personalize each message.</li>
                            <li class="mb-2"><i class="fe fe-check-circle me-1 text-success"></i> Sending to parent phone is recommended.</li>
                            <li><i class="fe fe-check-circle me-1 text-success"></i> Messages are sent via Termii and queued.</li>
                        </ul>
                    </div>
                </div>
            </div>
...
function loadTemplate(key) {
    ...
    document.getElementById('messageBody').value = templates[key];
    updateCharCount();
}

function updateCharCount() {
    const len = document.getElementById('messageBody').value.length;
    document.getElementById('charCount').textContent = len + ' / 320';
}
